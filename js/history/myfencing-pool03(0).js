...
function createPoolTable(id, poolData) {
  ...
  trHead.append(
    ...
    createEl("th", { class: "PTabH1s", text: "Streep " + streep + " - Time "   + time, colspan: 5, style: { textAlign: "left" } })
    //createEl("th", { class: "PTabH1t", text: "time "   + time,   colspan: 4, style: { textAlign: "left" } })
  );
  ...
  const arr1 = [{ text: "Athleta", class: "PTabH2CellA" },{ text: "Rank" }, { text: colChoice }];
  const arr3 = [{ class: "PTabH2Cempty" },
                { text: "TS" }, 
                { text: "RS" }, 
                { text: "Diff" },
                { class: "PTabH2Cempty" },{ text: "V" }, { text: "V/M" }];
  const arr2 = Array.from({ length: poolData.pool.length }, (_, i) => ({ text: (i + 1).toString() }));
  while (arr2.length < 7) { arr2.push({ class: "PTabH2Cempty" });}
  const headerCells = arr1.concat(arr2, arr3);
  ...
  let delta  = rowpool.value[rowpool.value.length - 2] - rowpool.value[rowpool.value.length - 1];
     ...
     tr.appendChild(createEl("td", {text: xx.toString(),style: { color: colore}}));
     tr.appendChild(createEl("td", { text: "", class: "PTabRCempty" }));
  
  // V V/M
  let vm = won / (rowpool.value.length - xx);
   // console.log("110 " + val);  
