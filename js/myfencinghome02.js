...
grid.addEventListener('click', e => {
  ...
  const target = 'pools'; // oppure 'pools', 'tableau' ecc. se vuoi cambiarlo
...
grid.addEventListener('click', e => {
  ...
  console.log('Hai cliccato sull\'evento con ID:', eventid);

  // Carica dinamicamente race1.js
  const script = document.createElement('script');
  script.src = `js/race01.js?id=${eventid}`; // se vuoi passare come query param
  script.onload = () => {
    console.log('race1.js caricato');

    // Se race1.js definisce una funzione globale es: loadRace(id)
    if (typeof window.loadRace === 'function') {
      window.loadRace(eventid);
    } else {
      console.error('Funzione loadRace non trovata in race1.js');
    }
  };
  script.onerror = () => {
    console.error('Errore nel caricamento di race1.js');
  };
...
function Gironi(eventId) {
  let poolContent = '<p><strong>Pools funzione Gironi</strong></p>';

 const div = document.createElement('div');
  div.textContent = `Pool dell'evento con ID: ${eventId}`;
  // return div; da verifcare in futuro
...
// Function createMenuBar
// ---------------------------------------------------------------------
//function createMenuBar11(defaultTarget = 'home') {
//  const menu = document.createElement('div');
//  menu.className = 'menu-bar';
//
//  const tabs = document.createElement('nav');
...
//  }
//
//  // Attiva la voce scelta (default: home)
//  setTimeout(() => {
//    document.querySelector(`.menu-option[data-target="${defaultTarget}"]`)?.click();
//  }, 0);
//}; 

function createMenuBar(defaultTarget = 'home', enabledTargets = []) {
  const menu = document.createElement('div');
  menu.className = 'menu-bar';
  ...
  itemMenuHome.forEach(opt => {
    ...
    // Se c'è una lista di target abilitati, e questo non c'è, lo disabiliti
...
//}
//
//function createMenuBar02(visibleTargets = ['home', 'ranking']) {
//  // Rimuovo vecchio menu se presente
//  if (currentMenu) currentMenu.remove();
//
//  const menu = document.createElement('div');
...
//}
//
//function createMenuBar(visibleTargets = ['home', 'ranking'], autoClick = true) {
//  // Rimuovo vecchio menu se presente
//  if (currentMenu) currentMenu.remove();
//
//  const menu = document.createElement('div');
...
//        if (currentMenu) currentMenu.remove();
//        const allTargets = itemMenu.map(item => item.target);
//        createMenuBar(allTargets, false); // ⚠️ niente autoClick
//        setTimeout(() => {
//          document.querySelector(`[data-target="${opt.target}"]`)?.click();
//        }, 0);
//      }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  //createMenuBar();
  // Mostra solo home e settings
  createMenuBar('home', ['home', 'ranking']);
  //createMenuBar('ranking');
});
